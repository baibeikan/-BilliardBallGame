...
void WavInFile::init()
{
    ...
    */
	...
	int dataSizeInBytes = getDataSizeInBytes();
	if (dataSizeInBytes > 0)
	{
		data = new char[dataSizeInBytes];
	}
    dataRead = 0;
}
...
WavOutFile::WavOutFile(int sampleRate, int bits, int channels)
{
	m_pWaveData = NULL;
	m_dataSize = 0;
	m_dataBufferSize = 0;
...
void WavOutFile::write(const float *buffer, int numElems)
{
	...
	if(m_dataBufferSize > m_dataSize + numBytes)
	{
		// 扩容
		char* pTemp = m_pWaveData;
		m_dataBufferSize = m_dataBufferSize+1024*8;
		m_pWaveData = new char[m_dataBufferSize];
		memset(m_pWaveData, 0, sizeof(char)*m_dataBufferSize);
		if(m_dataSize > 0)
		{
			memcpy(m_pWaveData, pTemp, m_dataSize);
		}
		SAFE_DELETE_ARR(pTemp);
	}
	memcpy(m_pWaveData+m_dataSize, temp, numBytes);
	m_dataSize += numBytes;
...
unsigned int WavOutFile::read(unsigned char *buffer, int maxElems)
{
	int bytes = min(m_dataSize-m_nBytesRead, maxElems);
	for (int i = 0; i < bytes; ++i)
	{
		buffer[i] = m_pWaveData[m_nBytesRead+i];
	}
	return bytes;
}
...
Wave::Wave(int sampleRate, /*/< Sample rate (e.g. 44100 etc) */ int bits, /*/< Bits per sample (8 or 16 bits) */ int channels /*/< Number of channels (1=mono, 2=stereo) */)
{
	m_pWavData = NULL;
	m_nDataSize = 0;
	m_nDataBufferSize = 0;
...
void Wave::init()
{
    ...
    */
	...
	unsigned int bytesDataSize = getDataSizeInBytes();
	m_pWavData = new char[bytesDataSize];
	m_nDataBufferSize = bytesDataSize;

	unsigned int MaxFileRead = 1024*1024;
	unsigned int nFileReadSize = 0;
	while(m_nDataSize < m_nDataBufferSize)
	{
		unsigned int readSize = min(bytesDataSize-m_nDataSize, MaxFileRead);
		if(readSize > 0)
		{
			readSize = fread(m_pWavData+m_nDataSize, 1, readSize, fptr);
			m_nDataSize += readSize;
		}
		else
		{
			break;
...
void Wave::fillInHeader(const uint sampleRate, const uint bits, const uint channels)
{
	// fill in the 'riff' part..
...
int Wave::read(unsigned char *buffer, int maxElems)
{
    int numBytes;
    uint afterDataRead;
    ...
    if (header.format.bits_per_sample != 8)
    {
        ST_THROW_RT_ERROR("Error: Wave::read(char*, int) works only with 8bit samples.");
    }
    assert(sizeof(char) == 1);
...
int Wave::read(short *buffer, int maxElems)
{
    unsigned int afterDataRead;
    int numBytes;
    int numElems;
...
/// Read data in float format. Notice that when reading in float format 
/// 8/16/24/32 bit sample formats are supported
int Wave::read(float *buffer, int maxElems)
{
    unsigned int afterDataRead;
    int numBytes;
    int numElems;
	...
	numBytes = min(m_nDataSize-m_nDataRead, maxElems * bytesPerSample);
	numElems = numBytes / bytesPerSample;
	char *temp = m_pWavData + m_nDataRead;
    // swap byte ordert & convert to float, depending on sample format
    switch (bytesPerSample)
    {
        case 1:
...
int Wave::readBuffer(char* buffer, unsigned int bytesSize, float fPitch)
{
	unsigned int dataSize = m_nDataSize;
	unsigned int numBytes = min(dataSize-m_nDataRead, bytesSize);
	if (numBytes > 0)
	{
		memcpy(buffer, m_pWavData+m_nDataRead, numBytes);
		m_nDataRead += numBytes;
	}
	return numBytes;
}

void Wave::setPitch(float pitch)
{
	//SoundTouch touch;
	//touch.setChannels(getNumChannels());
	//touch.setSampleRate(getSampleRate());

	//pitch = 10.0f;
	//touch.setPitchSemiTones(pitch);

	//unsigned int numBytes = m_nDataSize;

	//int nBitPerSample = getNumBits();
	//int nBytesPerSample = nBitPerSample / 8;
	//int maxElems = numBytes/nBytesPerSample;

	//float *pTemp = new float[BUFF_SIZE];
	//memset(pTemp, 0, sizeof(float)*BUFF_SIZE);
	//int nChannels = getNumChannels();

	//m_pUseWavData = new char[numBytes];
	//m_nUseWavBuffSize = numBytes;

	//char* buffer = m_pUseWavData;
	...
	//m_nDataRead = 0;
	//
	//while (m_nDataRead < numBytes)
	//{
	//	numElems = min((numBytes-m_nDataRead)/nBytesPerSample, BUFF_SIZE);
	//	numElems = WavToFloat(pTemp, numElems, m_pWavData+m_nDataRead, numBytes-m_nDataRead, nBitPerSample);
	//	m_nDataRead += numElems*nBytesPerSample;
	...
	//		if(numElems > 0)
	//		{
	//			numElems = FloatToWav(pTemp, numElems, buffer+m_nUseWavSize, numBytes-m_nUseWavSize, nBitPerSample);
	//			m_nUseWavSize += numElems;
	//		}
	//	} while (nSamples != 0);
	//}
	...
	//do 
	//{
	//	nSamples = touch.receiveSamples(pTemp, BUFF_SIZE/getNumChannels());
	//	numElems = nSamples*getNumChannels();
	//	if(numElems > 0)
	//	{
	//		numElems = FloatToWav(pTemp, numElems, buffer+m_nUseWavSize, numBytes-m_nUseWavSize, nBitPerSample);
	//		m_nUseWavSize += numElems;
...
void Wave::write(float* data, unsigned int dataSize)
{
	int nBitPerSample = getNumBits();
	int nBytesPerSample = getNumBits() / 8;
	int numTotalBytes = dataSize * nBytesPerSample;
	if(m_nDataBufferSize <= m_nDataSize + numTotalBytes)
	{
		// 需要创建数据
		int maxBufferSize = 1024 * 1024 * 16;
		int nDataBufferSize = m_nDataSize + numTotalBytes;
		bool bIsNew = false;
		if(m_nDataBufferSize >= maxBufferSize)
		{
			int offset = nDataBufferSize - m_nDataBufferSize + 1;
			memcpy(m_pWavData, m_pWavData+offset, sizeof(m_nDataSize-offset));
			m_nDataRead = m_nDataRead - offset;
			m_nDataSize = m_nDataSize - offset;
		}
		else
		{
			unsigned int newSize = min(maxBufferSize, nDataBufferSize+1);
			char* pWavTempData = m_pWavData;
			m_pWavData = new char[newSize];
			memset(m_pWavData, 0, sizeof(char)*newSize);

			if(nDataBufferSize > maxBufferSize)
			{
				int offset = nDataBufferSize - newSize + 1;
				memcpy(m_pWavData, m_pWavData+offset, sizeof(m_nDataSize-offset));
				m_nDataRead = m_nDataRead - offset;
				m_nDataSize = m_nDataSize - offset;
			}
			...
			{
				memcpy(m_pWavData, pWavTempData, m_nDataSize);
			}
			m_nDataBufferSize = newSize;
			SAFE_DELETE_ARR(pWavTempData);
		}
	}

	int nNumBytes = FloatToWav(data, dataSize, m_pWavData+m_nDataSize, m_nDataBufferSize-m_nDataSize, nBitPerSample);
	m_nDataSize += nNumBytes;
}

void Wave::write(char* data, unsigned int dataSize)
{
	int numTotalBytes = dataSize;
	if(m_nDataBufferSize <= m_nDataSize + numTotalBytes)
	{
		// 需要创建数据
		int maxBufferSize = 1024 * 1024 * 16;
	...
	memcpy(m_pWavData+m_nDataSize, data, dataSize);
	m_nDataSize = m_nDataSize + dataSize;
}

int Wave::readRIFFBlock()
{
...
uint Wave::getNumChannels() const
{
    return header.format.channel_number;
}


uint Wave::getNumBits() const
{
    return header.format.bits_per_sample;
}


uint Wave::getBytesPerSample() const
{
    return getNumChannels() * getNumBits() / 8;
}


uint Wave::getSampleRate() const
{
    return header.format.sample_rate;
}
...
uint Wave::getDataSizeInBytes() const
{
    return header.data.data_len;
}


uint Wave::getNumSamples() const
{
    if (header.format.byte_per_sample == 0) return 0;
    if (header.format.fixed > 1) return header.fact.fact_sample_len;
    return header.data.data_len / (unsigned short)header.format.byte_per_sample;
