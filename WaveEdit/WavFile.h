...
public:
	...
	WavOutFile(int sampleRate,          ///< Sample rate (e.g. 44100 etc)
	...
	unsigned int read(unsigned char *buffer, int maxElems);
...
class BytesBuffer
{
	...
	unsigned int m_nDataBufferSize;
	unsigned int m_nReadSize;
};


/// Class for reading WAV audio files.
class Wave: protected WavFileBase
{
public:
	/// Constructor: Opens the given WAV file. If the file can't be opened,
	/// throws 'runtime_error' exception.
	...
	Wave(int sampleRate,          ///< Sample rate (e.g. 44100 etc)
		int bits,                ///< Bits per sample (8 or 16 bits)
		int channels             ///< Number of channels (1=mono, 2=stereo)
		);
	/// Destructor: Closes the file.
	...
	int readBuffer(char* buffer, unsigned int bytesSize, float fPitch);
